...
pub.app = function(appName, callback) {
    ...
    appObj.getRoomOccupants=function(){
        return appObj.roomObj.getConnections()
    }

    appObj.getMates = function(roomName,callback){
        if (!appObj.isRoomSync(roomName)) {
            callback(new pub.util.ApplicationWarning("Attempt to request non-existent room name: '" + roomName + "'"));
            return;
        }

        callback(null,e.app[appName].room[roomName].clientList);
    }
